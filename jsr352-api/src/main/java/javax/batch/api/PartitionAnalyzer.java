 ...
 * each partition. If a PartitionCollector is configured on the step, the
 ...
 * on each thread processing a step partition, a single PartitionAnalyzer
 * instance runs on a single, consistent thread each time it is invoked.
 *
 */
public interface PartitionAnalyzer {
     ...
     * collector sends its payload. It receives the Externalizable object from
     * the collector as an input.
     *
     * @param data specifies the payload sent by a PartitionCollector.
     * @throws Exception is thrown if an error occurs.
     */
    void analyzeCollectorData(Externalizable data) throws Exception;

    /**
     * The analyzeStatus method receives control each time a partition ends. It
     * receives the batch and exit status strings of the partition as inputs.
     *
     * @param batchStatus specifies the batch status of a partition.
     * @param exitStatus specifies the exit status of a partition.
     * @throws Exception is thrown if an error occurs.
     */
    void analyzeStatus(String batchStatus, String exitStatus) throws Exception;
}
