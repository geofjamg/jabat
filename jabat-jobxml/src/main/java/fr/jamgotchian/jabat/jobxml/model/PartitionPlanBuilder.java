...
public class PartitionPlanBuilder {
    ...
    public PartitionPlanBuilder addProperties(int partition, Properties properties) {
        if (partition < 0 || partition >= partitionCount) {
            throw new JobXmlException("Wrong partition number " + partition);
        }
    ...
    public PartitionPlan build() {
        if (partitionCount <= 0) {
            throw new JobXmlException("Incorrect partition count " + partitionCount);
        }
        if (threadCount != null && threadCount <= 0) {
            throw new JobXmlException("Incorrect thread count " + threadCount);
        }
