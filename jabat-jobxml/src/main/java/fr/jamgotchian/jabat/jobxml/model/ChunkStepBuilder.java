...
public class ChunkStepBuilder extends StepBuilder<ChunkStepBuilder, ChunkStep> {
    ...
    public ChunkStepBuilder setCommitInterval(int commitInterval) {
        if (commitInterval < 1) {
            throw new JobXmlException("Chunk commit interval should be greater than 0");
        }
    ...
    public ChunkStepBuilder setBufferSize(int bufferSize) {
        if (bufferSize < 1) {
            throw new JobXmlException("Chunk buffer size is expected to be greater than 0");
        }
    ...
    public ChunkStepBuilder setRetryLimit(int retryLimit) {
        if (retryLimit < 0) {
            throw new JobXmlException("Chunk retry limit is expected to be greater or equal than 0");
        }
    ...
    public ChunkStepBuilder setSkipLimit(int skipLimit) {
        if (skipLimit < 0) {
            throw new JobXmlException("Chunk skip limit is expected to be greater or equal than 0");
        }
    ...
    public ChunkStep build() {
        check();
        if (reader == null) {
            throw new JobXmlException("Chunk reader artifact is not set");
        }
        if (processor == null) {
            throw new JobXmlException("Chunk processor artifact is not set");
        }
        if (writer == null) {
            throw new JobXmlException("Chunk writer artifact is not set");
        }
        CheckpointPolicy policy = getCheckpointPolicy();
        if (policy == CheckpointPolicy.CUSTOM && checkpointAlgo == null) {
            throw new JobXmlException("A checkpoint algorithm artifact is expected for a custom checkpoint policy");
        }
